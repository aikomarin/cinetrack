...
def url_pagina_catalogo(parametros, numero_pagina):
    query = parametros.copy()
    query["page"] = numero_pagina
    return f"?{query.urlencode()}"


def construir_paginacion_catalogo(parametros, page_obj):
    paginas = [
        {
            "numero": numero,
            "actual": numero == page_obj.number,
            "url": url_pagina_catalogo(parametros, numero),
        }
    ...
    return {
        "paginas": paginas,
        "primera": (
            url_pagina_catalogo(parametros, 1)
